...
//!
//! Bit layout of each source row byte: bit 0 = leftmost pixel (LSB-first).
...
pub fn draw_char(
    ...
    for (src_row, &bits) in glyph.iter().enumerate() {
        for col in 0..CHAR_W {
            let pixel = if (bits >> col) & 1 != 0 { fg } else { bg };

            // Each source row is painted `2 * scale` pixel rows tall,
            // and each source column is `scale` pixels wide.
            // Regular writes (not write_volatile): the framebuffer is Normal
            // RAM — the compiler can vectorise these, unlike write_volatile.
            for vr in 0..(2 * scale) {
                for hc in 0..scale {
