...
use crate::{AppId, Result, SysCall, SysCallResult, PID, TID};
    ...
    F: FnOnce(),
{
    ...
    // to happen for PID1.
    Ok(ProcessInit { app_id: PROCESS_KEY.with(|pk| *pk.borrow()).unwrap_or_else(default_process_key) })
}
    ...
    let thread_main = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            XOUS_SERVER_CONNECTION.with(|xsc| {
                ...
                match xous_connect_impl(server_address, &init.app_id) {
                    Ok(a) => {
                        *xsc = Some(a);
                        Ok(())
                    }
...
impl From<&ProcessInit> for [usize; 7] {
    fn from(init: &ProcessInit) -> [usize; 7] {
        [
            u32::from_le_bytes(init.app_id.0[0..4].try_into().unwrap()) as _,
            u32::from_le_bytes(init.app_id.0[4..8].try_into().unwrap()) as _,
            u32::from_le_bytes(init.app_id.0[8..12].try_into().unwrap()) as _,
            u32::from_le_bytes(init.app_id.0[12..16].try_into().unwrap()) as _,
            0,
            0,
            0,
        ]
...
impl TryFrom<[usize; 7]> for ProcessInit {
    ...
    fn try_from(src: [usize; 7]) -> core::result::Result<ProcessInit, crate::Error> {
        ...
        Ok(ProcessInit { app_id: AppId(key) })
    }
}
...
thread_local!(static PROCESS_KEY: RefCell<Option<AppId>> = RefCell::new(None));
...
fn default_process_key() -> AppId {
    std::env::var("XOUS_PROCESS_KEY")
        .map(|s| {
            let pk: ProcessKey = s.as_str().into();
            AppId(pk.0)
        })
        .unwrap_or(AppId([0u8; 16]))
}

pub fn set_process_key(new_key: &[u8; 16]) {
    PROCESS_KEY.with(|pk| *pk.borrow_mut() = Some(AppId(*new_key)));
}
...
fn xous_connect_impl(addr: SocketAddr, key: &AppId) -> core::result::Result<ServerConnection, ()> {
