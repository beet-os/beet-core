...
**Status: HEADLINE DONE — `cargo xtask qemu-smoke-net` boots BeetOS, runs the real userspace shell, connects over real TCP from the host, and gets a real `ifconfig` reply with the DHCP-assigned IP. `cargo xtask qemu-smoke-net-userspace` proves apps can listen *and* connect over TCP via `api/net`.** Phase 1 of the `os/console` service is in place and CI-tested; Phase 2 retirement of the local UART write is the only architectural cleanup left before this milestone closes. Real-NIC path remains an M7 extension.
