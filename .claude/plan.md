...
- [x] **Phase 2: retire the shell's direct UART writes** in favour of the service. `puts`/`putc` route through `os/console` (which owns the UART + TCP mirror); the direct UART write survives only as a boot-time fallback (probed via `Connect`-as-TryConnect on every call until the service registers) and in the shell's panic handler (mid-panic the IPC machinery can't be trusted). FB rendering stays in the shell — it owns the display while holding it (AcquireDisplay protocol), and FB bytes were never duplicated. Result: each output byte hits the UART exactly once (serial logs no longer show every line twice).
- [x] **`api/net/` — userspace TCP socket API.** `TcpListener` / `TcpStream` with eight new syscalls (`NetSocketCreate/Listen/Accept/Connect/Status/Send/Recv/Close`) backed by a 4-slot `UserSocket` table in `tcp.rs`. Per-socket RX/TX rings, full state machine (Listen/SynSent/SynReceived/Established/CloseWait/FinWait1/FinWait2/Closing/LastAck/TimeWait), ephemeral-port allocator, ARP-cache-driven active open (gateway routing for off-subnet peers via DHCP-learned router). Bytes ride inline in syscall args (max 32 B per call) — no user pointer crosses the EL0/EL1 boundary, PAN-safe by construction. The kernel console socket on port 2323 coexists in its own untouched datapath; userspace sockets share the segment builder and the ARP cache. `cargo xtask qemu-smoke-net-userspace` exercises both passive (`nettest-listen 7777` echo server) and active (`nettest-connect 10.0.2.2 <port>` to a host listener via slirp NAT) paths end-to-end.
  - [x] **Hardening pass**: per-PID socket ownership on every syscall (children inherit the listener's owner); socket reclamation on process exit (`cleanup_for_pid` hooked into TerminateProcess/TerminatePid); 5 s reaper for wind-down states (TimeWait/FinWait/Closing/LastAck/embryonic SynReceived) so lost final ACKs can't pin slots; honest receive windows (advertise actual RX-ring free space, partial-accept + peer-retransmit instead of silently dropping ACKed bytes, window-update ACK when a starved ring drains); sequence-gated FIN processing (dup FIN can't double-advance `rcv_nxt`) with re-ACK in CloseWait/TimeWait; stale-pending-child reclaim so a dead handshake can't wedge a listener; ephemeral-port uniqueness + cross-state port-in-use check on listen; ARP retries rate-limited to 4/s; kernel uptime ticks exposed via `NetGetInfo` slot 4 so userspace polls against real deadlines instead of CPU-speed-dependent yield-spin counts.
- [x] Shell command `ping <ip> [count]` — ICMP echo requests from `net_stack::ping_send` (id 0xBEE7, ARP-resolved next hop via the shared `next_hop_for`), replies captured into a single-slot `LAST_PONG`, exposed through two new syscalls (`NetPingSend`/`NetPingPoll`). RTT measured in userspace with the EL0-readable virtual counter (µs resolution). Covered in `qemu-smoke-net`: `ping 10.0.2.2 2` must come back 2/2.
- [ ] Real-NIC path (USB-C Ethernet / RP1 on Pi 5) + retransmission timer for lossy links. (M7 extension — needs hardware.)

**Status: CLOSED — `cargo xtask qemu-smoke-net` boots BeetOS, runs the real userspace shell over the TCP console (now the *only* steady-state output path — Phase 2 done), gets a real `ifconfig` reply and 2/2 ping replies from the gateway. `cargo xtask qemu-smoke-net-userspace` proves apps can listen *and* connect over TCP via `api/net`.** Real-NIC path remains as a hardware-dependent extension.
